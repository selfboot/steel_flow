...
| PRD | 状态 | 实现证据 |
|---|---|---|
| FR-001 型材选择 | 完成 | `CalculatorHomeView` 展示 13 类型材、双语尺寸简介、紧凑自适应网格、图形和无障碍名称。 |
| FR-002 单位输入 | 完成 | `Units.swift` 与 `CalculatorDraft` 保持 SI 真值并执行无损切换；公英制等价测试通过。 |
| FR-003 确定性计算 | 完成 | `CalculationEngine` 覆盖 13 种截面及无效几何；100 条版本化参考语料与全部截面公式测试全量回归。 |
...
| FR-007 PDF 报价 | 完成 | 中英文、A4/Letter、多页重复表头、末页总计安全区、固定浅色 PDF 调色、客户安全的税前行金额、Pro 公司/条款、PDFKit 真实文件预览、完整不可变报价快照和系统分享。 |
| FR-008 CSV | 完成 | 固定机器字段、UTF-8 BOM、面积/体积/损耗/价格来源/费用及项目汇总、独立数字/单位/币种列和系统分享。 |
| FR-009 本地化 | 完成 | 简体中文/English，手动 App 语言与独立报价语言分离；动态错误、复数、数字、日期、材料名及导出文件均显式遵循目标 locale，双语 key 集合自动校验。 |
| FR-010 数据可移植性 | 完成 | schema v3（继续读取 v1/v2）、20 MB 上限、稳定 JSON 编码、SHA-256 校验、导入前版本与项目/材料/客户/快照数量预览，可选恢复设置并复制导入；项目、价格簿、公司、客户、报价快照与偏好完整备份，所有语义校验通过后才原子提交。 |
...
- Debug：65 项测试通过（53 项单元测试 + 12 项 UI 测试），覆盖领域计算、100 条参考语料、自定义面积单位换算、所有计价单位损耗、币种舍入与迁移、Markup/Margin、非法价格、价格历史、完整备份与文件上限、旧数据兼容、备份原子性、数据库启动恢复、RevenueCat key 发布保护、Pro 权益撤销、免费项目限制、PDF 分页与客户字段隔离、CSV 汇总、完整报价快照、双语 key 一致性，以及核心导航、最大辅助字号、中英文小屏报价、中文长标签金额布局、独立报价语言和安全删除 UI 流程。
