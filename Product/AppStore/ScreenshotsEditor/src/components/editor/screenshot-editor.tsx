...
export function ScreenshotEditor() {
  ...
  // Locale and slide changes replace the screenshot nodes immediately, but
  // the browser may not have decoded every data-URI image before the next two
  // animation frames. Wait explicitly so the iPhone bezel and screenshots do
  // not disappear intermittently from later sizes in a bulk export.
    ...
    await Promise.all(
      images.map(async (image) => {
        ...
        try {
          ...
          // html-to-image will report a useful error if the asset is unusable.
        }
      }),
    );
  };
